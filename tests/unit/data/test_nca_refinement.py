...
class TestSingleSubjectNCA:
    ...
    def test_excludes_low_adj_r2(self) -> None:
        ...
        # Seeded noise (rng default_rng(0)) makes this deterministic: the
        # terminal-phase fit is poor, so the QC gate must produce a computable
        # result flagged as excluded with a sub-threshold adj_r².
        assert result is not None
        assert result.excluded
        assert result.excluded_reason is not None
        assert result.lambda_z_adj_r2 < 0.80
        assert "adj_r" in result.excluded_reason
