...
class TestStanEmitterBasic:
    ...
    def test_has_required_blocks(self) -> None:
        # Basic-emission cross-check on the default `_make_spec()` (no IIV);
        # the prior-path copy over `_one_cmt()` lives in
        # test_stan_emitter_priors.py::TestPriorsAreCompilableOutputShape.
        code = emit_stan(_make_spec())
        assert "data {" in code
...
class TestInitialEstimates:
    ...
    def test_default_priors_without_estimates(self) -> None:
        # Pins the no-initial-estimates default for log_ka; the structural
        # default-prior authority (log_CL) is
        # test_stan_emitter_priors.py::TestDefaultPriors.
        code = emit_stan(_make_spec())
