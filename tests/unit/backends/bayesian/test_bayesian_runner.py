...
class TestRunSuccessPath:
    """End-to-end run() with the harness subprocess mocked out.

    Patches asyncio.create_subprocess_exec so no live CmdStan is required:
    the fake harness writes a success response.json (whose path run() passes
    on argv) and reports a clean exit, driving run() through emit_stan,
    request serialisation, and _parse_response into a real BackendResult.
    """

    def test_run_returns_backend_result(self, tmp_path: Path) -> None:
        def _spawn_factory(*args: object, **kwargs: object) -> object:
            response_path = Path(str(args[3]))
            response_path.write_text(
                json.dumps(
                    {
                        "schema_version": "1.0",
                        "status": "success",
                        "result": _mock_result_dict("test_classical"),
                        "session_info": {},
                    }
                )
            )
        ...
        async def _mock_spawn(*args: object, **kwargs: object) -> object:
            return _spawn_factory(*args, **kwargs)

        data_path = tmp_path / "data.csv"
        data_path.write_text("ID,TIME,DV\n1,0,0\n")
        runner = BayesianRunner(work_dir=tmp_path)
        with patch(
        ...
        assert result.model_id == "test_classical"
        assert result.backend == "bayesian_stan"
        assert result.converged is True
        assert "CL" in result.parameter_estimates

    def test_run_accepts_and_ignores_test_data_path(self, tmp_path: Path) -> None:
        """test_data_path is accepted for BackendRunner-protocol conformance
        and ignored (mirrors the Nlmixr2/BayesianRunner accept-and-ignore
        contract documented in run()'s docstring): passing it must not error
        and must yield the same result as omitting it."""
        ...
        test_data_path.write_text("ID,TIME,DV\n2,0,0\n")
        runner = BayesianRunner(work_dir=tmp_path)

        def _run(with_test_data: bool) -> Any:
            with patch(
                "apmode.backends.bayesian_runner.asyncio.create_subprocess_exec",
                side_effect=_mock_spawn,
            ):
                return asyncio.run(
                    runner.run(
                        ...
                        seed=7,
                        data_path=data_path,
                        test_data_path=test_data_path if with_test_data else None,
                    )
                )

        baseline = _run(with_test_data=False)
        with_test = _run(with_test_data=True)
        assert with_test.model_id == baseline.model_id == "test_classical"
        assert with_test.backend == baseline.backend == "bayesian_stan"
        assert with_test.converged == baseline.converged is True
