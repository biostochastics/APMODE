...
ka2, frac, MT_1, MT_2, RD2_1, RD2_2, weight_1, V, CL, ...) live in
``DSLSpec.initial``; structural modules carry only topology (n, k).
"""
...
def _base_spec(absorption: object, initial: dict[str, float] | None = None) -> DSLSpec:
    merged_initial = {"V": 70.0, "CL": 5.0}
    if initial:
        merged_initial.update(initial)
    return DSLSpec(
        model_id="t",
...
def _disposition_fixed_spec(
    absorption: object, initial: dict[str, float] | None = None
) -> DSLSpec:
...
_SUMIG_INITIAL = {"MT_1": 2.0, "MT_2": 6.0, "RD2_1": 0.5, "RD2_2": 1.0, "weight_1": 0.6}
...
class TestASTConstruction:
    ...
    def test_parallel_first_order_basic(self) -> None:
        p = ParallelFirstOrder()
        assert p.type == "ParallelFirstOrder"
    ...
    def test_structural_param_names_erlang(self) -> None:
        spec = _base_spec(Erlang(n=3), initial={"ktr": 1.5})
    ...
    def test_structural_param_names_parallel(self) -> None:
        spec = _base_spec(ParallelFirstOrder(), initial={"ka1": 1.5, "ka2": 0.5, "frac": 0.6})
        names = spec.structural_param_names()
        assert {"ka1", "ka2", "frac"}.issubset(names)

    def test_structural_param_names_sumig(self) -> None:
        spec = _base_spec(SumIG(k=2), initial=_SUMIG_INITIAL)
        names = spec.structural_param_names()
...
class TestGrammarParse:
    def test_parse_erlang(self, parser: Lark) -> None:
        src = """
        model {
            absorption: Erlang(n=3, ktr)
            distribution: OneCmt(V)
            elimination: Linear(CL)
            variability: IIV(params=[CL], structure=diagonal)
            observation: Proportional(sigma_prop=0.1)
            initial: { ktr = 1.5, V = 70.0, CL = 5.0 }
        }
        """
    ...
    def test_parse_parallel_first_order(self, parser: Lark) -> None:
        src = """
        model {
            absorption: ParallelFirstOrder(ka1, ka2, frac)
            distribution: OneCmt(V)
            elimination: Linear(CL)
            variability: IIV(params=[CL], structure=diagonal)
            observation: Proportional(sigma_prop=0.1)
            initial: { ka1 = 1.5, ka2 = 0.5, frac = 0.6, V = 70.0, CL = 5.0 }
        }
        """
        assert parser.parse(src) is not None
    ...
    def test_parse_sumig(self, parser: Lark) -> None:
        src = """
        model {
            absorption: SumIG(k=2, MT_1, MT_2, RD2_1, RD2_2, weight_1)
            distribution: OneCmt(V)
            elimination: Linear(CL)
            variability: IIV(params=[CL], structure=diagonal)
            observation: Proportional(sigma_prop=0.1)
            initial: {
                MT_1 = 2.0, MT_2 = 6.0, RD2_1 = 0.5, RD2_2 = 1.0, weight_1 = 0.6,
                V = 70.0, CL = 5.0
            }
        }
        """
...
class TestValidatorErlang:
    def test_n_must_be_positive_int(self) -> None:
        spec = _base_spec(Erlang(n=0), initial={"ktr": 1.0})
...
class TestValidatorSumIG:
    def test_k_capped_at_2_in_v07(self) -> None:
        spec = _disposition_fixed_spec(SumIG(k=3), initial=_SUMIG_INITIAL)
        errors = validate_dsl(spec, lane=Lane.DISCOVERY)
    ...
    def test_mt_ordering_enforced(self) -> None:
        spec = _disposition_fixed_spec(
            ...
            initial={**_SUMIG_INITIAL, "MT_1": 6.0, "MT_2": 2.0},
        )
        errors = validate_dsl(spec, lane=Lane.DISCOVERY)
    ...
    def test_disposition_fixed_unblocks_sumig(self) -> None:
        spec = _disposition_fixed_spec(SumIG(k=2), initial=_SUMIG_INITIAL)
        errors = validate_dsl(spec, lane=Lane.DISCOVERY)
    ...
    def test_weight_unit_interval(self) -> None:
        spec = _disposition_fixed_spec(SumIG(k=2), initial={**_SUMIG_INITIAL, "weight_1": 1.0})
        errors = validate_dsl(spec, lane=Lane.DISCOVERY)
        assert any(e.constraint == "unit_interval" for e in errors)
...
class TestConvertTransitToErlang:
    def test_validate_requires_transit(self) -> None:
        spec = _base_spec(FirstOrder(), initial={"ka": 1.0})
    ...
    def test_apply_drops_terminal_ka(self) -> None:
        spec = _base_spec(Transit(n=4), initial={"ktr": 2.0, "ka": 1.0})
        t = ConvertTransitToErlang(n=3)
        new_spec = apply_transform(spec, t)
        assert isinstance(new_spec.absorption, Erlang)
        assert new_spec.absorption.n == 3
        assert new_spec.initial["ktr"] == 2.0  # ktr inherited from Transit
        assert "ka" not in new_spec.initial
...
class TestAddParallelRoute:
    ...
    def test_apply_creates_parallel(self) -> None:
        spec = _base_spec(FirstOrder(), initial={"ka": 1.5})
        t = AddParallelRoute(ka2=0.3, frac=0.6)
        new_spec = apply_transform(spec, t)
        assert isinstance(new_spec.absorption, ParallelFirstOrder)
        assert new_spec.initial["ka1"] == 1.5
        assert new_spec.initial["ka2"] == 0.3
        assert new_spec.initial["frac"] == 0.6
...
class TestSetSumIGComponents:
    ...
    def test_validate_mt_ordering(self) -> None:
        spec = _disposition_fixed_spec(SumIG(k=2), initial={**_SUMIG_INITIAL, "weight_1": 0.5})
    ...
    def test_apply_updates_components(self) -> None:
        ...
        assert new_spec.initial["MT_1"] == 1.5
        assert new_spec.initial["MT_2"] == 8.0
        assert new_spec.initial["weight_1"] == 0.7
