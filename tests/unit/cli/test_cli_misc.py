...
"""CLI-layer tests for the remaining top-level ``apmode`` commands: --help /
--version smoke tests, ``validate``, ``datasets``, ``explore`` (+ policy
forwarding), ``report``, ``doctor``, ``ls``, ``policies``, and the
skill-documented ``bundle rocrate`` aliases.

Split out of the former monolithic ``test_cli.py``. These tests exercise the
CLI through ``typer.testing.CliRunner``; heavy pipeline work is mocked.
"""
