...
SUITE_A_DIR = Path(__file__).parent.parent.parent.parent / "benchmarks" / "suite_a"
...
class TestBordaCompositeAggregation:
    ...
    def test_borda_ranks_by_sum_of_ranks(self) -> None:
        # Concordant winner/loser with a distinct middle, so the three rank
        # sums are all different (2 / 4 / 6) and the ordering is unambiguous.
        # VPC concordance = 1 - mean(|coverage - 0.90|), higher wins (rank 1).
        # NPE proxy = sqrt(cwres_mean² + (cwres_sd - 1)²), lower wins (rank 1).
        best = _make_result(
            model_id="best",
            backend="nlmixr2",
            cwres_mean=0.0,
            cwres_sd=1.0,  # NPE proxy = 0.0  → npe rank 1
            vpc_coverage={"p5": 0.90, "p50": 0.90, "p95": 0.90},  # concordance 1.0 → vpc rank 1
        )
        middle = _make_result(
            model_id="middle",
            backend="jax_node",
            cwres_mean=0.2,
            cwres_sd=1.1,  # NPE proxy ≈ 0.224 → npe rank 2
            vpc_coverage={"p5": 0.85, "p50": 0.85, "p95": 0.85},  # concordance 0.95 → vpc rank 2
        )
        worst = _make_result(
            model_id="worst",
            backend="agentic_llm",
            cwres_mean=0.5,
            cwres_sd=1.5,  # NPE proxy ≈ 0.707 → npe rank 3
            vpc_coverage={"p5": 0.75, "p50": 0.75, "p95": 0.75},  # concordance 0.85 → vpc rank 3
        )
        result = _rank([worst, middle, best], gate3=self._BORDA_VPC_NPE)

        assert result.composite_method == "borda"
        # Rank sums: best = 1+1 = 2, middle = 2+2 = 4, worst = 3+3 = 6.
        assert [m.candidate_id for m in result.ranked_candidates] == ["best", "middle", "worst"]
        by_id = {m.candidate_id: m for m in result.ranked_candidates}
        assert by_id["best"].composite_score == pytest.approx(2.0)
        assert by_id["middle"].composite_score == pytest.approx(4.0)
        assert by_id["worst"].composite_score == pytest.approx(6.0)
        assert by_id["best"].component_scores == pytest.approx({"vpc": 1.0, "npe": 1.0})
        assert by_id["worst"].component_scores == pytest.approx({"vpc": 3.0, "npe": 3.0})
