...
"""Reusable reproducibility-bundle builders for the test suite.

These helpers construct small, realistic sealed bundles on disk suitable
for RO-Crate projection tests. They were previously defined inside
``tests/unit/rocrate/_fixtures.py`` and imported cross-tier by the golden
and integration suites; they now live here so those imports do not reach
into a unit-test module.
...
- Seal the bundle so that ``RoCrateEmitter`` accepts it.
- Keep file counts small so golden snapshots stay reviewable.

The sentinel digest is computed by
``tests.unit.rocrate._fixtures._digest_bundle`` — a deliberately bespoke
helper that mirrors ``apmode.bundle.emitter._DIGEST_EXCLUDED_RELATIVE_PATHS``
and is kept in the RO-Crate unit-test package as the lockstep pin. It is
imported lazily inside :func:`build_submission_bundle` to avoid an import
cycle (``_fixtures`` re-imports :func:`build_submission_bundle` from here).
"""
...
def build_submission_bundle(
    ...
    *,
    ...
    #
    # ``_digest_bundle`` lives in the RO-Crate unit-test package as the
    # deliberate lockstep pin against the emitter's digest-exclusion set;
    # it is imported lazily here to avoid an import cycle (that module
    # re-imports this builder).
    from tests.unit.rocrate._fixtures import _digest_bundle
