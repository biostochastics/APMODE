...
def _empirical_bootstrap_draws(
    ...
    # Guard against a diagonal that is itself non-finite or zero — use a
    # floor so the downstream samples have at least some variance; the
    # caller should treat this as a hard degradation signal.
    diag = np.where(np.isfinite(diag) & (diag > 0), diag, 1e-6)
    std = np.sqrt(diag)
    noise = rng.standard_normal(size=(n_draws, theta.shape[0]))
    return theta[np.newaxis, :] + noise * std[np.newaxis, :]
...
def laplace_draws(
    ...
    *,
    ...
    Returns:
        Array of shape ``(n_draws, p)`` with draws rounded to
        ``float64``. Rows are independent.
    ...
    if _cov_is_ill_conditioned(cov):
        ...
        return _empirical_bootstrap_draws(theta, cov, n_draws, rng)

    try:
        # method="cholesky" fails fast for non-PSD; falls back to "svd"
        # which tolerates small negative eigenvalues by treating them as zero.
